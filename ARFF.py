...
def genAttr(attribute):
	...
	if re.match("numeric",attr,re.I):
		return (name, Numeric())
	elif re.match("string",attr,re.I):
		return (name, String())
...
class ARFF:
	...
	def __str__(self):
		output = []
		output.extend(["%% %s" % comment for comment in self.comment])
	...
	def writePARF(self,file):
		...
		for row in self.data:
			new_row = map(lambda x: "?" if x == "NA" else x, row)
			output.extend([','.join(partial_row) for partial_row in self.splitrow(new_row)])
		output.append("")
		file.write('\n'.join(output))
